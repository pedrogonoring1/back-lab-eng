...
import { create } from '../repositories/UsuarioRepository';
...
const createUsuario = async (request, response) => {
  ...
  if (createdEndereco != 'Address already exists') {
    ...
    if (createdUsuario == null || createdUsuario == undefined)
      return response.status(500).json({ error: 'O cadastro do usuário já existe no banco de dados.' });
